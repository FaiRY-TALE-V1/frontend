...
const FairytaleForm = () => {
  ...
  useEffect(() => {
    const generateStory = async () => {
      try {
        ...
        // API 호출
        const response = await apiService.generateCompleteStory({
        ...
        <div className="max-w-md text-center">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button
              ...
              className="px-6 py-3 text-gray-600 transition-all duration-300 border border-gray-300 rounded-xl hover:bg-gray-50"
            >
              처음으로
            </button>
          </div>
  ...
  const progressPercentage = ((currentScene + 1) / story.story.scenes.length) * 100;
                ...
                {currentSceneData.image_url ? (
                  <img
                    ...
                    onError={(e) => {
                      ...
                      target.src = `https://via.placeholder.com/400x400/6366F1/FFFFFF?text=장면+${currentScene + 1}`;
                    }}
                  />
                ) : (
                  <div className="flex items-center justify-center w-full shadow-2xl h-96 rounded-3xl ring-4 ring-white/20 bg-gradient-to-br from-purple-400 to-pink-400">
                    <div className="text-center text-white">
                      ...
                      <p className="text-xl font-semibold">장면 {currentScene + 1}</p>
                    </div>
                  </div>
                )}
                {/* 이미지 오버레이 효과 */}
