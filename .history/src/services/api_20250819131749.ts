...
const handleApiError = (error: any, defaultMessage: string): never => {
  if (error.response) {
    ...
    if (status === 404) {
      throw new Error("요청한 기능을 찾을 수 없습니다. 서버 설정을 확인해주세요.");
    } else if (status === 500) {
...
export const apiService = {
  ...
  getThemes: async (): Promise<{ themes: Theme[] }> => {
    try {
      ...
      handleApiError(error, "테마 목록을 불러올 수 없습니다.");
    }
  },
  ...
  generateCompleteStory: async (
    ...
    try {
      ...
      handleApiError(error, "스토리를 생성할 수 없습니다.");
    }
  },
  ...
  healthCheck: async (): Promise<any> => {
    try {
      ...
      handleApiError(error, "API 서버 상태를 확인할 수 없습니다.");
    }
  },
};
...
export const qwenImageEditService = {
  ...
  generateCharacterFromPhoto: async (
    ...
    try {
      ...
      throw new Error("캐릭터 생성에 실패했습니다. 서버 연결을 확인해주세요.");
    }
  },
  ...
  generatePersonalizedScene: async (
    ...
    try {
      ...
      throw new Error("장면 생성에 실패했습니다. 서버 연결을 확인해주세요.");
    }
  },
  ...
  addPersonalizedText: async (
    ...
    try {
      ...
      throw new Error("텍스트 추가에 실패했습니다. 서버 연결을 확인해주세요.");
    }
  },
  ...
  editImageWithPrompt: async (
    ...
    try {
      ...
      throw new Error("이미지 편집에 실패했습니다. 서버 연결을 확인해주세요.");
    }
  },
};
