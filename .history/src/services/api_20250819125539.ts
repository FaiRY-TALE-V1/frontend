...
export const qwenImageEditService = {
  ...
  generateCharacterFromPhoto: async (
    ...
    try {
      ...
      const response = await api.post("/qwen/edit-image", editRequest);
      return response.data;
    } catch (error) {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            editedImage: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==", // 더미 이미지
  ...
  generatePersonalizedScene: async (
    ...
    try {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            editedImage: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==",
